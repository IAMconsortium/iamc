...
The package comes with small example data set `example_REMIND` supposed to be checked against the configuration 'examplePROJECT'. To show the functionality of the package some mistakes have been introduced into the data set. To find these mistakes you can feed the object to the check function `iamCheck`:

```{r, echo=TRUE}
library(iamc)
iamCheck(example_REMIND, cfg="examplePROJECT")
```
...
Setting `cfg="examplePROJECT"` in the example above makes sure, that the data is analyzed based on a rule set specified in 'examplePROJECT'. It is possible to use other project settings instead. One way is to load an existing cfg project settings and customize it. In the following example we take the existsing examplePROJECT config, introduce a unknown variable `Population_ERROR` by renaming `Population`. Second example sets a new maximum value for "FE"  to 200:

```{r, echo=TRUE}
# first example
# load cfg
cfg <- iamProjectConfig("examplePROJECT")
# modify cfg
