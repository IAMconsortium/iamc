...
The currently given default validation data can be inspected by calling iamReferenceData().
...
### Output-PDF
For each run of consistency checks the user can specify a pdf which summarizes the results of the checks and the validation. 
...
To add for instance a 42 check which makes sure that values to do lie between 42 and 2*42, you can write a function `check42` following the rules mentioned above and run `iamCheck` with `globalenv = TRUE`:
